...
import { getThemes, likeTheme } from "@/utils/db/actions";
...
export default function Home() {
	...
	return (
		...
		<div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 w-screen">
			<UploadTheme />
			<div className="grid sm:grid-cols-4 gap-4 shrink-0 basis-auto sm:w-3/4">
				{
					themes.map((theme, index) => (
						<div key={`theme-${index}`} className="bg-base-200 sm:min-w-64 min-w-full min-h-16 border-base-300 border p-2 shadow-2xl">
							...
							<SignedIn>
								{
									userData.likes?.includes(theme.id)
										? (
											<Button onClick={() => onClick(theme.id, index)} className="mt-3 text-lg font-medium text-base-100-content hover:text-accent-content ">
												{theme.likes}
